...
export default {
  ...
  mounted() {
    ...
    if (this.dragHandle) {
      ;[...this.$el.querySelectorAll(this.dragHandle)].forEach((dragHandle) => {
        dragHandle.dataset.dragHandle = this._uid
      })
    }
    ...
    if (this.dragCancel) {
      ;[...this.$el.querySelectorAll(this.dragCancel)].forEach(
        (cancelHandle) => {
          cancelHandle.dataset.dragCancel = this._uid
        }
      )
    }
  },
  ...
  methods: {
    ...
    bodyDown(ev) {
      ...
      if (
        this.dragHandle &&
        target.dataset.dragHandle !== this._uid.toString()
      ) {
        return
      }
      ...
      if (
        this.dragCancel &&
        target.dataset.dragCancel === this._uid.toString()
      ) {
        return
      }
