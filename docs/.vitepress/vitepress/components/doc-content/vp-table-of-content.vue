...
<template>
  <aside ref="container" class="toc-wrapper">
    <nav class="toc-content">
      ...
      <el-anchor :offset="70" :bound="120">
        <el-anchor-link v-for="{ link, text, children } in headers" :key="link" :href="link" :title="text">
          ...
          <template v-if="children" #sub-link>
            <el-anchor-link v-for="{ link: childLink, text: childText } in children" :key="childLink" :href="childLink"
              :title="text">
      ...
      /> -->
      <p v-if="false" class="text-14px font-300 color-$text-color-secondary">
        {{ sponsor.sponsoredBy }}
      </p>
      <sponsors-button v-if="false" class="sponsors-button mt-4 w-100%" />
      <sponsor-right-big-logo-list v-if="false" />
      <sponsor-right-logo-small-list v-if="false" />
      <sponsor-right-text-list v-if="false" />
    </nav>
  </aside>
</template>
