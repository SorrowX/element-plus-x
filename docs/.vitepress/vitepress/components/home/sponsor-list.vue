...
<template>
  <h2 v-if="false" class="text-center mb-4 text-xl">
    {{ sponsorLang[sponsorType] }}
  </h2>
  <div v-if="false" class="grid gap-1 sponsor-list platinum">
    <a v-for="(sponsor, i) in sponsors" :key="i" :class="['sponsor flex px-4 rounded-md', sponsor.className]"
      :href="sponsor.url" target="_blank" @click="onItemClick(sponsor)">
